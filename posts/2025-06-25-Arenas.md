...
tags: memory management, arenas
author: me
language: English
myst:
    html_meta:
        "property=og:title": "Arenas"
        "language": "en"
---
...
[Ryan Fleury](https://www.rfleury.com/) has a blog on substack [Untangling Lifetimes: The Arena Allocator - by Ryan Fleury](https://www.rfleury.com/p/untangling-lifetimes-the-arena-allocator). 

## Notes
### Memory Management and Arenas (Ryan Fleury)

***Source: [Everyone is doing memory management wrong. feat. Ryan Fleury | S2 E02](https://www.youtube.com/watch?v=UeJPyuVxL-o)***
...
On the other side is the Heap Allocator ([malloc](https://en.cppreference.com/w/c/memory/malloc) and [free](https://en.cppreference.com/w/c/memory/free.html)), which lets you dynamically give a lifetime to a variable. 
...
So Arenas can either have a single large reservation of memory and the ability to grow, maybe by chaining. Contrary to stack, where you can run out of space ([stackoverflow](https://en.wikipedia.org/wiki/Stack_overflow)).
...
The interface of arena allocator is `push` and `push` at the base. Not `malloc` and `free`, so you cannot have arbitrary overlapping lifetime inside the arena.  
You basically push new bytes at the end and pop them off the back.

Many new languages have explicit allocator passing. (example: [std.array_list.ArrayListAligned.init - Zig Documentation](https://ziglang.org/documentation/0.14.1/std/#std.array_list.ArrayListAligned.init) or [std::vector<T,Allocator>::vector - cppreference.com](https://en.cppreference.com/w/cpp/container/vector/vector.html))

```{code-block} zig
:caption: ArrayList with allocator
:linenos:
...
// reference: https://pismice.github.io/HEIG_ZIG/docs/allocators/
```

```{code-block} cpp
:caption: Vector with allocator
:linenos:
...
{
    explicit _GLIBCXX20_CONSTEXPR vector(const allocator_type &__a) _GLIBCXX_NOEXCEPT : _Base(__a) {}
}
...
In C++ [`std::pmr::monotonic_buffer_resource`](https://en.cppreference.com/w/cpp/memory/monotonic_buffer_resource.html) acts like an arena

```{code-block} cpp
:caption: Monotonic Buffer Resource
:linenos:
...
```
Example: 
1. in a game, you might have a per frame arena. So every frame, you clear the arena, reset the allocation back to the minimum, and they you push all kind of dynamic stuff. And the at the beginning of the next frame 
2. in web server or similar, you can have per request arena. Input to the request, results to respond with, etc all that could be pushed to arena and cleared after the data has been sent over the network.
3. per thread scratch arenas, which are kind of like stack for that thread.

Similar, to the dynamic push and pop style of the arenas, it is possible to have dynamic allocation onto the stack in C, using [`alloca`](https://learn.microsoft.com/en-us/cpp/c-runtime-library/reference/alloca) .


```{admonition} OS does the Allocations
:class: important
...
#### Pointer Stability
An arena can be thought of a [slice](https://go.dev/ref/spec#Slice_types) (from the language [Go](https://go.dev/)) of memory. But with an important caveat: when a slice resizes in Go, it is not guaranteed that the pointer to an element in that slice remains valid. Similar with `std::vector` in C++. The pointer is not stable after the resize due to realloc.  
...
See [STL/stl/inc/vector at main · microsoft/STL · GitHub](https://github.com/microsoft/STL/blob/main/stl/inc/vector#L875) how `std::vector` is resized when capacity is reached.
...
The more you move towards solving generic problems, you kind of start writing your `malloc` and `free`. 
...
## Others
### Malloc
[newlib/libc/stdlib/\_mallocr.c at master · bminor/newlib](https://sourcegraph.com/github.com/bminor/newlib@master/-/blob/newlib/libc/stdlib/_mallocr.c?L2334) how actual malloc is implemented using `srbk` and `brk` syscalls.

An excellent read on inner workings of malloc:
- [Malloc\_tutorial.pdf - MarwanBurelle Laboratoire Système et Sécurité de l’EPITA (LSE)](https://wiki-prog.infoprepa.epita.fr/images/0/04/Malloc_tutorial.pdf)

## References:
- [Everyone is doing memory management wrong. feat. Ryan Fleury | S2 E02 - YouTube](https://www.youtube.com/watch?v=UeJPyuVxL-o&feature=youtu.be)
- [Memory Allocation Strategies - Part 2 - gingerBill](https://www.gingerbill.org/article/2019/02/08/memory-allocation-strategies-002/)
- [FreeRTOS-Kernel/portable/MemMang at main · FreeRTOS/FreeRTOS-Kernel](https://github.com/FreeRTOS/FreeRTOS-Kernel/tree/main/portable/MemMang)
- [Game Engine Stream: Creating a Linear Memory Allocator](https://www.youtube.com/live/FqeanKu3NzI)
- [Generating Aligned Memory - Embedded Artistry](https://embeddedartistry.com/blog/2017/02/22/generating-aligned-memory/)
